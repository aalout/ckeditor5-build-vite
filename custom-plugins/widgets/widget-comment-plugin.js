import {Plugin, ButtonView, toWidget, Widget, toWidgetEditable} from 'ckeditor5';;
    ...
    init() {
        ...
        editor.conversion.for( 'editingDowncast' ).elementToElement( {
            ...
            view: ( modelElement, { writer: viewWriter } ) => {
                ...
                const productCardFlex1 = viewWriter.createContainerElement( 'div', {
                    class: 'product-card-flex-1'
                } );
        
                const paragraph = viewWriter.createContainerElement( 'p' );
                writer.appendText( 'Следует отметить, что реализация намеченных плановых заданий способствует подготовке и реализации модели развития. Учитывая ключевые сценарии поведения, высокое качество позиционных исследований требует от нас анализа инновационных методов управления процессами. Но новая модель организационной деятельности является качественно новой ступенью переосмысления внешнеэкономических политик.', paragraph );
        ...
        editor.ui.componentFactory.add( 'insertProductCard', locale => {
            ...
            view.on('execute', () => {
                editor.model.change(writer => {
                    ...
                    const productCardTitle = writer.createElement('productCardTitle');
                    writer.appendText('Следует отметить, что реализация намеченных плановых заданий способствует подготовке и реализации модели развития.', productCardTitle);
        
                    // Create the description element.
                    const productCardDescription = writer.createElement('productCardDescription');
                    writer.appendText('Учитывая ключевые сценарии поведения, высокое качество позиционных исследований требует от нас анализа инновационных методов управления процессами.', productCardDescription);
